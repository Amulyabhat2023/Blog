...
async function savePosts(posts) {
  try {
    await window.storage.set(STORAGE_KEY, JSON.stringify(posts));
  } catch (e) {
  ...
  return (
    <div className={`landing ${visible ? "fade-in" : ""}`}>
      ...
      <header className="landing-header">
        <div className="logo-mark">✦ CAMPUS CHRONICLES ✦</div>
      ...
      <section className="hero">
        <div className="hero-tag">EST. 2024 · A COLLEGE STORY</div>
        <h1 className="hero-title">
          <span className="line1">Life Between</span>
          <span className="line2">the <em>Lectures</em></span>
        </h1>
        <p className="hero-desc">
          A visual journal documenting the beautifully chaotic, unforgettable world of college — the late nights, the friendships, the firsts, and everything in between.
        </p>
      ...
      <section className="features">
        {[
          { icon: "📸", title: "Visual Stories", desc: "Every moment captured — from sunrise study sessions to midnight celebrations." },
          { icon: "🎓", title: "Campus Life", desc: "Chronicles of friendships forged, dreams chased, and memories made." },
          { icon: "✨", title: "Curated Moments", desc: "Handpicked highlights from the most vibrant years of your life." },
        ].map((f, i) => (
      ...
      <footer className="landing-footer">
        <span>© Campus Chronicles 2024</span>
        <span className="footer-divider">✦</span>
        <span>Where memories live forever</span>
      </footer>
    </div>
  );
}
  ...
  return (
    <div className="dashboard">
      ...
      <header className="dash-header">
        ...
        <div className="logo-mark small">✦ CAMPUS CHRONICLES</div>
              ...
              <div className="post-card" key={post.id}>
                <div className="post-card-thumb">
                  {post.media[0]?.type === "video"
                    ? <video src={post.media[0].data} className="thumb-media" muted />
        ...
        {posts.map((post, i) => (
          <div className="viewer-card" key={post.id} style={{ animationDelay: `${i * 0.08}s` }} onClick={() => openPost(post)}>
            <div className="viewer-card-media">
              {post.media[0]?.type === "video"
                ? <video src={post.media[0].data} className="viewer-thumb" muted />
              ...
              {selected.media.length > 1 && (
                <div className="lb-thumbs">
                  {selected.media.map((m, i) => (
                    ...
                    >
                      {m.type === "video"
                        ? <video src={m.data} className="lb-thumb-media" muted />
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body { background: var(--black); color: var(--text); font-family: var(--ff-body); overflow-x: hidden; }
